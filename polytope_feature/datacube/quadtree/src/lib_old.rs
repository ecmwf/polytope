...
impl QuadTreeNode {
    ...
    // #[getter]
    // fn children(&self) -> Vec<Py<QuadTreeNode>> {
    //     self.children.clone() // Cloning `Py<QuadTreeNode>` is cheap since it’s just a reference count increment
    // }
    ...
    //     let py_list = PyList::empty(py);
    //     for child in &self.children {
    //         // let py_child = Py::new(py, child).unwrap(); // Use Py::new to avoid cloning
    //         let py_child = PyCell::new(py, child).unwrap().into(); // Use PyCell to wrap
    //         py_list.append(py_child).unwrap();
    //     }
    //     py_list.into()
    // }
    ...
    // #[getter]
    // fn children<'py>(&self, py: Python<'py>) -> Py<PyList> {
    //     let py_list = PyList::empty(py);
    //     for child in &self.children {
    //         let py_child: Py<PyAny> = PyCell::new(py, child)
    //             .map(|cell| cell.into()) // Convert PyCell to a Python object
    //             .unwrap_or_else(|_| py.None()); // Handle errors safely
    //         py_list.append(py_child).unwrap();
    //     }
    //     py_list.into()
    // }
