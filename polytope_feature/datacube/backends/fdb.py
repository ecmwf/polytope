...
class FDBDatacube(Datacube):
    ...
    def get_fdb_requests(
        ...
        # If request node has no children, we have a leaf so need to assign fdb values to it
            ...
            if not requests.children[0].children:
                # TODO: the children are now GridNodes
                print("ARE HERE WHERE WE EXPECT??")
                print(requests.children[0].indexes)
                (path, current_start_idxs, fdb_node_ranges, lat_length) = self.get_leaf_grid_node_values(requests, leaf_path)
                print("AND GOT HERE NOW??")
                (
                    original_indices,
                    sorted_request_ranges,
                    fdb_node_ranges,
                ) = self.sort_fdb_request_ranges(current_start_idxs, lat_length, fdb_node_ranges, True)
                fdb_requests.append((path, sorted_request_ranges))
                fdb_requests_decoding_info.append((original_indices, fdb_node_ranges))
                pass
            elif not requests.children[0].children[0].children:
                # TODO: We have a GridNode instead of a normal node as a grandchild
                for c in requests.children:
    ...
    def get_leaf_grid_node_values(self, requests, leaf_path=None):
        # here we only work with GridNodes so actually it's a completely "flat" leaf
        if leaf_path is None:
            leaf_path = {}

        # TODO: find the nearest lat/lon

        tree_length = len(requests.children)
        current_start_idxs = [False] * tree_length
        fdb_node_ranges = [False] * tree_length

        for i, c in enumerate(requests.children):
            # Add each child to the fdb_node_ranges
            fdb_node_ranges[i] = [[c]]
            # TODO: unmap each lat/lon successively with the right datacube axis
            current_start_idxs[i] = [c.indexes]
            # TODO: update the leaf path
            # key_value_path = {c.axes[0]: (c.values[0],), c.axes[1]: (c.values[1],)}
            # leaf_path.update(key_value_path)

        # TODO: remove the leaf_path_copy leaves we do not need
        # TODO: return everything like before
        leaf_path_copy = deepcopy(leaf_path)
        return (leaf_path_copy, current_start_idxs, fdb_node_ranges, tree_length)
    ...
    def sort_fdb_request_ranges(self, current_start_idx, lat_length, fdb_node_ranges, is_grid_node=False):
        if not is_grid_node:
            ...
            for i in range(lat_length):
                ...
                for j in range(len(old_interm_start_idx)):
                    ...
                    for interm_fdb_nodes_obj in interm_fdb_nodes[j]:
                        interm_fdb_nodes_obj.values = tuple([interm_fdb_nodes_obj.values[k]
