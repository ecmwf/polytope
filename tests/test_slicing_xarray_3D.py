...
class TestSlicing3DXarrayDatacube():
    ...
    def test_polytope(self):
        ...
        request = Request(ConvexPolytope(["step", "level"], points), 
                Select("date", ["2000-01-01"])
                )
        result = self.API.retrieve(request)
        self.xarraydatacube.get(result)
    ...
    def test_swept_concave_polygon(self):
        # Tests what happens when we slice a concave shape which is swept across a path and see if concavity is lost or not
    ...
    def test_intersection_point_disk_polygon(self):
        ...
        r1 = math.cos(math.pi/12)*(8-4*math.sqrt(3))+sys.float_info.epsilon # need small perturbation to make up for rounding errors
        r2 = 3*math.cos(math.pi/12)*(math.sqrt(3)-2)*(8-4*math.sqrt(3))/(4*math.sqrt(3)-7)
        request = Request(
            Disk(["level", "step"],[0, 0], [r1, r2]),
            Select("date",["2000-01-01"])
        )
        result = self.API.retrieve(request)
