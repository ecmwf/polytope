...
def test_get_options_without_dynamic_grid():
    ...
    assert axis_config[0].axis_name == "number", f"Expected axis_name 'number', got '{axis_config[0].axis_name}'"
...
def test_get_options_without_dynamic_grid_no_flag():
    ...
    # Verify basic structure
    assert len(axis_config) == 1, f"Expected 1 axis config, got {len(axis_config)}"
    assert axis_config[0].axis_name == "latitude", f"Expected axis_name 'latitude', got '{axis_config[0].axis_name}'"
...
def test_get_options_with_dynamic_grid_missing_deps():
    ...
    assert len(axis_config) == 1, f"Expected 1 axis config, got {len(axis_config)}"
    assert axis_config[0].axis_name == "values", f"Expected axis_name 'values', got '{axis_config[0].axis_name}'"
    
    print("✓ Test passed: get_polytope_options gracefully handles missing optional dependencies")
