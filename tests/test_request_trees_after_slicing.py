...
class TestIndexTreesAfterSlicing:
    ...
    def test_add_child(self):
        ...
        axis1.name = "lat"
        request2.create_child(axis1, tuple([4.1]))
        assert request2.leaves[0].axis.name == "lat"
        assert request2.leaves[0].values == tuple([4.1])
        axis2 = IntDatacubeAxis()
        axis2.name = "level"
        # Test getting child
        assert request1.create_child(axis2, tuple([3.0])).axis.name == "level"
        assert request1.create_child(axis2, tuple([3.0])).values == tuple([3.0])
    ...
    def test_remove_branch(self):
        ...
        request1 = request.create_child(axis1, tuple([1.0]))
        request2 = request1.create_child(axis2, tuple([0.0]))
