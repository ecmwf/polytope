...
class TestSlicing4DXarrayDatacube:
    ...
    def test_ellipsoid(self):
        ...
        request = Request(ellipsoid, Select("date", ["2000-01-01"]), Select("variable", ["param"]))
        result = self.API.retrieve(request)
        assert len(result.leaves) == 7
    ...
    def test_not_all_axes_defined(self):
        with pytest.raises(AxisUnderdefinedError):
            request = Request(Box(["step", "level"], [3, 10], [6, 11]), Select("variable", ["param"]))
            result = self.API.retrieve(request)
            result.pprint()

    def test_not_all_axes_exist(self):
        with pytest.raises(KeyError):
            request = Request(Box(["weather", "level"], [3, 10], [6, 11]), Select("variable", ["param"]))
            result = self.API.retrieve(request)
            result.pprint()
