...
class TestFloatType:
    ...
    def test_slicing_span(self):
        request = Request(
            Span("lat", 4.1, 4.3), Select("long", [4.1]), Select("alt", [4.1])
        )
    ...
    def test_slicing_point(self):
        request = Request(
            Select("lat", [4.1]), Select("long", [4.1]), Select("alt", [4.1])
        )
        result = self.API.retrieve(request)
        result.pprint()
    ...
    def test_slicing_very_close_point(self):
        request = Request(
            Select("lat", [4.1]), Select("long", [4.0999919, 4.1]), Select("alt", [4.1])
        )
        result = self.API.retrieve(request)
        result.pprint()
        assert len(result.leaves) == 1
    ...
    def test_slicing_points_higher_precision(self):
        request = Request(
            Select("lat", [4.12]), Select("long", [4.1]), Select("alt", [4.1])
        )
        result = self.API.retrieve(request)
        result.pprint()
    ...
    def test_slicing_points_empty_span_higher_precision(self):
        request = Request(
            Span("lat", 4.11, 4.12), Select("long", [4.1]), Select("alt", [4.1])
        )
        result = self.API.retrieve(request)
        assert result.leaves[0].is_root()
    ...
    def test_slicing_points_span_higher_precision(self):
        request = Request(
            Span("lat", 4.09, 4.12), Select("long", [4.1]), Select("alt", [4.1])
        )
        result = self.API.retrieve(request)
