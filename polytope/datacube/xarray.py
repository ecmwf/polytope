...
from ..utility.combinatorics import validate_axes, unique
...
class XArrayDatacube(Datacube):
    ...
    def __init__(self, dataarray: xr.DataArray, options = {}):
        ...
        for name, values in dataarray.coords.variables.items():
            if name in dataarray.dims:
                ...
                if not values.dtype.type in _mappings:
                    raise ValueError(f"Could not create a mapper for index type {values.dtype.type} for axis {name}")
                if name in self.options.keys():
                    # The options argument here is supposed to be a nested dictionary like {"latitude":{"Cyclic":range}, ...}
                    # TODO: would it be faster if instead we just add an option when it's cyclic and then evaluate to true or false? Maybe there is a better/faster way of accessing options
    ...
    def get(self, requests: DatacubeRequestTree):
        for r in requests.leaves:
            ...
            # TODO: Here, once we flatten the path, we want to remap the values on the axis to fit the datacube... need to see about this
    ...
    def get_indices(self, path: DatacubePath, axis, lower, upper):
        ...
        for i in range(len(search_ranges)):
            ...
            # Now the indexes_between are values on the cyclic range so need to remap them to their original values before returning them
