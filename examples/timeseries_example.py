...
class Test():
    ...
    def test_slice_shipping_route(self):
        ...
        minmin = np.min([np.min(temps1), np.min(temps2), np.min(temps3),np.min(temps4),np.min(temps5),np.min(temps6),np.min(temps7),np.min(temps8)])
        maxmax = np.max([np.max(temps1), np.max(temps2),np.max(temps3),np.max(temps4),np.max(temps5),np.max(temps6),np.max(temps7),np.max(temps8)]) 
